...
- `MessageScroller` is the headless root. It owns opening position, follow-output, anchoring, scroll commands, and visibility state.
...
The styled frame and layout container. It fills its parent, so use it inside a height-constrained layout, within a `MessageScroller`. Remaining props spread onto the frame `div`.
