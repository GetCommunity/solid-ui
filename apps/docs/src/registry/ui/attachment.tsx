...
const Attachment = <T extends ValidComponent = "div">(
  ...
  const others = omit(merged as AttachmentOptions, "as", "class", "state", "size", "orientation")
...
const AttachmentMedia = <T extends ValidComponent = "div">(
  ...
  const others = omit(merged as AttachmentMediaProps, "as", "class", "variant")
...
const AttachmentContent = <T extends ValidComponent = "div">(
  ...
  const merged = merge({ as: "div" as const }, props)
  const others = omit(merged as AttachmentContentProps, "as", "class")

  return (
    <Polymorphic
      as={props.as}
      class={cn("cn-attachment-content min-w-0 max-w-full flex-1", props.class)}
...
const AttachmentTitle = <T extends ValidComponent = "span">(
  ...
  const merged = merge({ as: "span" as const }, props)
  const others = omit(merged as AttachmentContentProps, "as", "class")
...
const AttachmentTrigger = <T extends ValidComponent = "button">(
  ...
  const merged = merge({ as: "button" as const }, props)
  const others = omit(merged as AttachmentTriggerProps, "as", "class", "type")
...
const AttachmentGroup = <T extends ValidComponent = "div">(
  ...
  const merged = merge({ as: "div" as const }, props)
  const others = omit(merged as AttachmentGroupProps, "as", "class")
